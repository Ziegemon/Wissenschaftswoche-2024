...
func _physics_process(delta: float) -> void:
	...
	if rolling == false:
		...
		if attacking == false:
			if is_on_floor():
				if Input.is_action_just_pressed("attack_1_p1"):
					animation_player.play("attack_1")
					attack_cooldown.start(0.5)
					attacking = true
				if Input.is_action_just_pressed("attack_2_p1"):
					animation_player.play("attack_2")
					attack_cooldown.start(1)
					attacking = true
				if Input.is_action_just_pressed("double_attack_p1"):
	...
	if Input.is_action_just_pressed("roll_p1") and can_roll:
		$roll_timer.start()
		$roll_cooldown.start()
		can_roll = false
		rolling = true
